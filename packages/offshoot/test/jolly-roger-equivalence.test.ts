...
import {buildReplacements, processAllFiles} from "./reference-create-jolly-roger.js";
...
async function fetchTemplate(): Promise<{dir: string; sha: string} | undefined> {
	try {
...
describeOnline("acceptance: equivalence with create-jolly-roger", () => {
	...
	async function offshootOutput(name: string, force: boolean): Promise<Map<string, VirtualFile>> {
	...
	function referenceOutput(name: string): Map<string, VirtualFile> {
		...
		const files = readTree(target, {skipDirs: ["node_modules", ".git"], skipFiles: [], exclude: []});
		return new Map(files.map((f) => [f.path, f]));
	}
	...
	function compare(
		...
		for (const [path, referenceFile] of reference) {
			...
			const originalLines = readFileSync(join(templateDir, ...path.split("/")), "utf8").split("\n");
	...
	it("differs from the reference in exactly one place: the occurrence it misses", async () => {
		const {extra} = compare(await offshootOutput(NAME, true), referenceOutput(NAME));
	...
	it("fixes that occurrence", async () => {
		...
		expect(reference.get(path)?.content.toString("utf8")).toContain("jolly-roger-e2e-node.log");
		expect(ours.get(path)?.content.toString("utf8")).toContain("my-app-e2e-node.log");
	});
	...
	it("scaffolds end to end, exactly as the acceptance line reads", async () => {
		...
		const read = (...parts: string[]) => readFileSync(join(result.dir, ...parts), "utf8");

		expect(read("package.json")).toContain('"name": "my-app"');
		expect(read("web", "package.json")).toContain('"name": "my-app-web"');
		expect(read("contracts", "package.json")).toContain('"name": "my-app-contracts"');
		expect(read("web", "src", "web-config.json")).toContain('"name": "My App"');
		expect(read("web", "src", "web-config.json")).toContain('"title": "My App"');
		expect(read("web", "e2e", "tests", "home.e2e.ts")).toContain("/my app/i");
		expect(read("web", "e2e", "tests", "home.e2e.ts")).toContain('img[alt="My App"]');
		expect(read("README.md")).toContain('"my-app-contracts/abis/GreetingsRegistry.js"');
		...
		const state = JSON.parse(read(".offshoot.json")) as {template: string; ref: string; sourceName: string};
	...
	it("doctor reports the template as usable, and names the risky spot", async () => {
		...
		const result = await doctor({cwd: templateDir, sourceName: "jolly-roger", log: quietLog});
