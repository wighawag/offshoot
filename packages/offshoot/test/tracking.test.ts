...
function templateWithVariant() {
	...
	writeFile(
		template.dir,
		'src/full-only.ts',
		'export const full = "demo-template";\n',
	);
...
describe('tracking a non-default branch', () => {
	it('scaffolds from `<template>#variant/full` and records it as the tracked ref', async () => {
		...
		expect(state.ref).toBe(
			git(['rev-parse', 'variant/full'], template.dir).trim(),
		);
		...
		expect(readFile(project.dir, 'src/full-only.ts')).toBe(
			'export const full = "my-app";\n',
		);
	});
	...
	it('switches variants with `update --ref`, and keeps tracking the new one', async () => {
		...
		expect(readFile(project.dir, 'src/full-only.ts')).toContain(
			'export const v = 9;',
		);
	});
