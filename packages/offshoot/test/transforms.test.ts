...
describe('change-case v4 -> v5 mapping', () => {
	...
	for (const variant of DEFAULT_VARIANTS) {
		it(`${variant} matches change-case@4`, () => {
			for (const name of names) {
				const v4 = (
					changeCase4 as unknown as Record<string, (s: string) => string>
				)[variant];
	...
	it('produces the expected variants for the jolly-roger token', () => {
		...
		const map = Object.fromEntries(
			pairs.map((p) => [p.variant, [p.from, p.to]]),
		);
...
describe('rename transform', () => {
	...
	it('renames files and nested directories', () => {
		...
		const out = applyRename(input, 'demo-template', 'my-app').files.map(
			(f) => f.path,
		);
		expect(out).toEqual([
			'src/my-app/my-app.ts',
			'src/my-app/nested/MyApp.spec.ts',
		]);
	});
	...
	it('regression: the utf-8 round-trip that corrupted binaries in create-jolly-roger', () => {
		...
		const naive = Buffer.from(
			bytes.toString('utf-8').split('demo-template').join('my-app'),
			'utf-8',
		);
		expect(naive.length).not.toBe(bytes.length);

		const safe = applyRename([file('x.png', bytes)], 'demo-template', 'my-app')
			.files[0];
	...
	it('preserves the executable bit', () => {
		const input = [
			file('scripts/demo-template.sh', '#!/bin/sh\necho demo-template\n', {
				executable: true,
			}),
		];
		const out = applyRename(input, 'demo-template', 'my-app').files[0];
...
describe('the uniqueness gate', () => {
	it('passes when the transform is round-trippable', () => {
		...
		expect(() =>
			assertRoundTrip(input, 'demo-template', 'my-app'),
		).not.toThrow();
	});
	...
	it('reports the offending occurrences with file and line', () => {
		...
		try {
			...
			expect(
				error.occurrences.some(
					(o) => o.path === 'docs/readme.md' && o.line === 3,
				),
			).toBe(true);
		}
	});
...
describe('patterns transform', () => {
	it('applies context-anchored pairs in order', () => {
		...
		const out = transform.apply(
			[
				file(
					'package.json',
					'{"name": "jolly-roger", "description": "Jolly Roger rules"}',
				),
			],
			{name: 'my-app', title: 'My App'},
			ctx,
		);
		expect(out[0]?.content.toString()).toBe(
			'{"name": "my-app", "description": "My App rules"}',
		);
	});
	...
	it('does not touch unanchored occurrences', () => {
		...
		const out = transform.apply(
			[file('a.md', 'the jolly-roger flag')],
			{},
			ctx,
		);
...
describe('template transform', () => {
	it('expands placeholders only in the included files', () => {
		const transform = createTemplateTransform({
			type: 'template',
			include: ['src/**/*.ts'],
		});
		const out = transform.apply(
	...
	it('supports conditionals with the {{ }} tags', () => {
		...
		expect(
			transform
				.apply([file('a.md', source)], {docs: true}, ctx)[0]
				?.content.toString(),
		).toBe('yes\n');
		expect(
			transform
				.apply([file('a.md', source)], {docs: false}, ctx)[0]
				?.content.toString(),
		).toBe('no\n');
	});
	...
	it('treats a bare {{ name }} as output, not as a discarded statement', () => {
		...
		const out = transform.apply(
			[
				file(
					'a.txt',
					'name={{ name }} explicit={{= name }} dotted={{ it.name }}\n',
				),
			],
			{name: 'my-app'},
			ctx,
		...
		expect(out[0]?.content.toString()).toBe(
			'name=my-app explicit=my-app dotted=my-app\n',
		);
	});
	...
	it('refuses to run without an include list', () => {
		expect(() =>
			createTemplateTransform({type: 'template', include: []}),
		).toThrow(/opt-in/);
	});
});
...
describe('path interpolation', () => {
	it('substitutes simple variables', () => {
		expect(interpolatePath('src/{{name}}.ts', {name: 'my-app'})).toBe(
			'src/my-app.ts',
		);
		expect(interpolatePath('src/{{ name }}/index.ts', {name: 'my-app'})).toBe(
			'src/my-app/index.ts',
		);
	});

	it('refuses an expansion that would create directories', () => {
		expect(() => interpolatePath('src/{{name}}.ts', {name: 'a/b'})).toThrow(
			/path separator/,
		);
	});

	it('refuses an unknown variable', () => {
		expect(() => interpolatePath('src/{{nope}}.ts', {name: 'x'})).toThrow(
			/not an answer/,
		);
	});
});
...
describe('binary detection', () => {
	...
	it('sniffs content when the extension is unknown', () => {
		...
		expect(looksBinary('blob', Buffer.from([0x00, 0x01, 0x02, 0xff]))).toBe(
			true,
		);
	});

	it('treats a token-bearing binary as binary', () => {
		expect(looksBinary('data.bin', binaryWithToken('demo-template'))).toBe(
			true,
		);
	});
});
