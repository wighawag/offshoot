...
describe("running project commands inside a template repository", () => {
	function configuredTemplate() {
		return createTemplateRepo({
			...defaultTemplateFiles(),
			"offshoot.config.json": JSON.stringify({sourceName: "demo-template"}, null, 2),
		});
	}
	...
	it("refuses update, check, rename and eject", async () => {
		...
		await expect(update({cwd, log: quietLog})).rejects.toThrow(/TEMPLATE repository/);
		await expect(check({cwd, log: quietLog})).rejects.toThrow(/TEMPLATE repository/);
		await expect(rename({cwd, newName: "x", log: quietLog})).rejects.toThrow(/TEMPLATE repository/);
		await expect(eject({cwd, log: quietLog})).rejects.toThrow(/TEMPLATE repository/);
	});

	it("points the author at `offshoot doctor` instead", async () => {
		const template = configuredTemplate();
		await expect(update({cwd: template.dir, log: quietLog})).rejects.toThrow(/offshoot doctor/);
	...
	it("exits non-zero from the CLI, so CI notices", () => {
		...
		let status = 0;
		try {
			execFileSync(process.execPath, [CLI, "update"], {cwd: template.dir, stdio: "pipe"});
		} catch (err) {
	...
	it("falls back to the generic message for a zero-config template", async () => {
		...
		await expect(update({cwd: template.dir, log: quietLog})).rejects.toThrow(/No \.offshoot\.json found/);
	});
});
...
describe("a template that is itself an offshoot project", () => {
	...
	function upstreamFiles(): Record<string, string> {
		return {
			"package.json": JSON.stringify({name: "upstream-template", version: "0.0.0"}, null, 2) + "\n",
	...
	it("never ships its own .offshoot.json to the projects it generates", async () => {
		...
		expect(trackedFiles(downstream.dir).filter((f) => f.endsWith(".offshoot.json"))).toEqual([".offshoot.json"]);
	});
	...
	it("keeps updating cleanly from the middle template afterwards", async () => {
		...
		writeFile(middle.dir, "src/added.ts", 'export const added = "demo-template";\n');
		...
		expect(readFile(downstream.dir, "src/added.ts")).toBe('export const added = "final-app";\n');
		// Still exactly one state file, still its own.
		expect(
			(JSON.parse(readFile(downstream.dir, ".offshoot.json")) as {answers: {name?: string}}).answers.name,
		).toBe("final-app");
	});
});
