...
describe('7. a project updates using only offshoot', () => {
	it('runs the offshoot CLI directly, with no create-* package present', async () => {
		...
		expect(trackedFiles(project.dir).some((f) => f.startsWith('create-'))).toBe(
			false,
		);
		...
		expect(readFile(project.dir, 'src/index.ts')).toContain(
			'export const added = 1;',
		);
	...
	it('exposes `check` with a non-zero exit when behind', async () => {
		...
		const upToDate = execFileSync(process.execPath, [CLI, 'check'], {
			cwd: project.dir,
			encoding: 'utf8',
		});
		...
		let stdout = '';
		try {
			stdout = execFileSync(process.execPath, [CLI, 'check'], {
				cwd: project.dir,
				encoding: 'utf8',
			});
		} catch (err) {
...
describe('8. binary files survive scaffold and update', () => {
	it('keeps them byte-identical, including one containing the source token', async () => {
		...
		const plainBinary = Buffer.from([
			0x89, 0x50, 0x4e, 0x47, 0x00, 0x01, 0xff, 0xfe,
		]);
		...
		expect(
			readBytes(project.dir, 'assets/my-app.png').equals(tokenBinary),
		).toBe(true);
		expect(readBytes(project.dir, 'assets/logo.png').equals(plainBinary)).toBe(
			true,
		);
	...
	it('preserves the executable bit through scaffold and update', async () => {
		...
		expect(
			git(['ls-files', '-s', 'scripts/run-my-app.sh'], project.dir),
		).toContain('100755');
		expect(readFile(project.dir, 'scripts/run-my-app.sh')).toContain(
			'echo my-app v2',
		);
	});
});
...
describe('9. file and directory renaming', () => {
	it('renames nested directories and the files inside them', async () => {
		const template = createTemplateRepo({
			...
			'src/demo-template/deep/nested/DemoTemplate.ts':
				'export class DemoTemplate {}\n',
			'packages/demo-template-core/package.json':
				'{"name": "demo-template-core"}\n',
		});
		const project = await scaffoldFrom(template.dir);
		...
		expect(readFile(project.dir, 'src/my-app/deep/nested/MyApp.ts')).toBe(
			'export class MyApp {}\n',
		);
	});
});
...
describe('10. the round-trip count check', () => {
	...
	it('runs again on update, catching a collision introduced by a NEW template ref', async () => {
		...
		await expect(
			update({cwd: project.dir, log: quietLog}),
		).rejects.toBeInstanceOf(RoundTripError);

		// The project is left exactly as it was, on its own branch.
		expect(isClean(project.dir)).toBe(true);
		expect(git(['rev-parse', '--abbrev-ref', 'HEAD'], project.dir).trim()).toBe(
			'main',
		);
	});
});
...
describe('11. zero-config scaffolding', () => {
	it('works on a template that has never heard of offshoot', async () => {
		...
		expect(readFile(project.dir, 'src/index.ts')).toBe(
...
describe('12. offshoot rename', () => {
	it('leaves the next update conflict-free', async () => {
		...
		expect(readFile(project.dir, 'package.json')).toContain(
			'"name": "better-name"',
		);
...
describe('13. the delimiter asymmetry', () => {
	it('interpolates {{name}} in paths while contentTags is <% %>', async () => {
			...
			'offshoot.config.json': JSON.stringify(
				{
					...
					transforms: [
						{type: 'rename'},
						{type: 'template', include: ['src/**/*.ts', '*.vue']},
					],
				},
				null,
				2,
		...
		expect(readFile(project.dir, 'App.vue')).toBe(
