...
describe("template configuration", () => {
	it("composes patterns then rename", async () => {
		const template = createTemplateRepo({
			"offshoot.config.json": JSON.stringify({
				...
				transforms: [
					{type: "patterns", patterns: [{from: "SPDX-License-Identifier: MIT", to: "SPDX-License-Identifier: MIT"}]},
					{type: "rename"},
				],
			}),
		...
		expect(readFile(project.dir, "LICENSE.ts")).toBe("// SPDX-License-Identifier: MIT\nconst p = 'my-app';\n");
	});
	...
	it("honours a js config with a function-valued replacement", async () => {
		...
		expect(readFile(project.dir, "package.json")).toBe('{"name": "my-app", "author": "Ronan"}\n');
...
export default defineConfig({
	...
	it("registers a custom Transform supplied by the template", async () => {
		...
		const project = await scaffoldFrom(template.dir);
		expect(readFile(project.dir, "README.md")).toBe("<!-- built by my-app -->\n# My App\n");
	});

	it("excludes files that must never reach the generated project", async () => {
		const template = createTemplateRepo({
			"offshoot.config.json": JSON.stringify({sourceName: "demo-template", exclude: [".github/**", "AUTHORING.md"]}),
			"package.json": '{"name": "demo-template"}\n',
		...
		const project = await scaffoldFrom(template.dir);
		expect(exists(project.dir, ".github/workflows/template-ci.yml")).toBe(false);
	...
	it("commits skip-listed files untransformed", async () => {
		...
		expect(readFile(project.dir, "pnpm-lock.yaml")).toBe("packages:\n  demo-template: 1\n");
	});

	it("seeds skipIfExists files once and never updates them", async () => {
		const template = createTemplateRepo({
			"offshoot.config.json": JSON.stringify({sourceName: "demo-template", skipIfExists: [".env.example"]}),
			"package.json": '{"name": "demo-template"}\n',
	...
	it("supports a configurable template branch name", async () => {
		const template = createTemplateRepo({
			"offshoot.config.json": JSON.stringify({sourceName: "demo-template", branch: "upstream-template"}),
			"package.json": '{"name": "demo-template"}\n',
		});
		const project = await scaffoldFrom(template.dir);

		expect(git(["branch", "--list"], project.dir)).toContain("upstream-template");
	...
	it("reads the config from the fetched ref, so authors can evolve it", async () => {
		...
		writeFile(template.dir, "offshoot.config.json", JSON.stringify({sourceName: "demo-template", skipFiles: ["keep.txt"]}));
...
describe("offshoot doctor", () => {
	...
	it("flags occurrences that look like ordinary words", async () => {
		...
		expect(result.occurrences.some((o) => o.bare && o.path === "README.md")).toBe(true);
	});

	it("warns about a token that is too short or a single word", async () => {
		const template = createTemplateRepo({"package.json": '{"name": "app"}\n'}, "app");
		const result = await doctor({cwd: template.dir, log: quietLog});
	...
	it("fails when the token appears nowhere", async () => {
		const template = createTemplateRepo({"package.json": '{"name": "something-else"}\n'});
		const result = await doctor({cwd: template.dir, log: quietLog});
	...
	it("fails the round-trip probe for a name the author asks about", async () => {
		...
		const result = await doctor({cwd: template.dir, name: "widget", log: quietLog});
...
describe("offshoot check and eject", () => {
	...
	it("cuts the link permanently", async () => {
		...
		await expect(update({cwd: project.dir, log: quietLog})).rejects.toThrow(/\.offshoot\.json/);
	});
});
...
describe("update guards", () => {
	it("refuses a dirty working tree", async () => {
		...
		await expect(update({cwd: project.dir, log: quietLog})).rejects.toThrow(/not clean/);
	});
