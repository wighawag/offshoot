...
import type {Answers, PatternsSpec, Transform, TransformContext, VirtualFile} from "../types.js";
	...
	return {
		...
		apply(files: VirtualFile[], answers: Answers, _ctx: TransformContext): VirtualFile[] {
			const pairs = spec.patterns.map((p) => {
				...
				if (typeof to !== "string") {
					throw new Error(`patterns transform: replacement for "${p.from}" did not produce a string`);
				}
			...
			return files.map((file) => {
				...
				if (!file.binary) {
					...
					if (after !== before) next = {...next, content: Buffer.from(after, "utf8")};
				}
