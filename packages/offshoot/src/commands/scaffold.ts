...
export async function scaffold(
	...
	try {
		...
		if (prepared.configFile)
			log.debug(`using ${prepared.configFile} from the template`);
		...
		if (!dirArg) {
			...
			if (typeof name !== 'string' || name === '') {
				throw new Error(
					'No target directory given and no `name` answer to derive one from.',
				);
			}
			dirArg = name;
		}
		...
		const state: OffshootState = {
			...
			answers,
			...(prepared.config.branch !== 'template'
		...
		log.info(
			`Transforming template (${prepared.config.sourceName} -> ${answers.name ?? '?'}) ...`,
		);
		...
		if (useGit && eject) {
			...
			g.commit(
				target,
				`Initial commit from ${state.template}@${state.ref.slice(0, 7)}`,
			);
		} else if (useGit) {
		...
		log.info('');
		log.info(
			`Created ${dirArg} from ${prepared.source.id}@${prepared.sha.slice(0, 7)}`,
		);
		if (useGit && eject) {
			log.info(
				`  no template link: this is a plain repository on "${mainBranch}".`,
			);
			log.info(`  \`offshoot update\` will not work here, by design.`);
		} else if (useGit) {
			log.info(
				`  branch "${mainBranch}" is yours; branch "${branch}" tracks the template.`,
			);
			log.info(
				`  run \`offshoot update\` later to merge template improvements.`,
			);
		}
 ...
 */
export function settleLockfile(
	root: string,
	...
	if (result.refreshed) {
		log.info(
			`  updated ${result.packageManager?.lockfile} (removed ${removedDependencies.join(', ')})`,
		);
		return;
	}
...
function defaultBranchName(cwd: string): string {
	const configured = g
		.gitTry(['config', 'init.defaultBranch'], cwd)
		.stdout.trim();
...
function preferredMainBranch(cwd: string, templateBranch: string): string {
	...
	if (candidate === templateBranch)
		return templateBranch === 'main' ? 'trunk' : 'main';
	return candidate;
}
