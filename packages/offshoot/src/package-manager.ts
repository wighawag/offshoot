...
export function detectPackageManager(root: string): PackageManager | undefined {
	...
	if (declared)
		return {name: declared, lockfile: lock?.file, via: 'packageManager field'};
...
export function refreshLockfile(root: string, log: Logger): RefreshResult {
	...
	if (!pm.lockfile)
		return {
			refreshed: false,
			packageManager: pm,
			reason: 'no lockfile in the template',
		};
	...
	if (!command) {
		return {
			refreshed: false,
			packageManager: pm,
			reason: `${pm.name} has no lockfile-only install`,
		};
	}
...
export function staleLockfileWarning(
