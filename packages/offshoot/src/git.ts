...
export function git(args: string[], cwd: string): string {
	...
	if (result.status !== 0)
		throw new GitError(args, result.status, result.stdout, result.stderr);
...
export function branchExists(cwd: string, branch: string): boolean {
	return (
		gitTry(['show-ref', '--verify', '--quiet', `refs/heads/${branch}`], cwd)
			.status === 0
	);
}
...
export function hasIdentity(cwd: string): boolean {
	return (
		gitTry(['config', 'user.name'], cwd).status === 0 &&
		gitTry(['config', 'user.email'], cwd).status === 0
	);
}
...
export function merge(
	...
	const result = gitTry(
		[...args, 'merge', '--no-edit', '-m', message, branch],
		cwd,
	);
	if (result.status === 0)
		return {ok: true, conflicted: [], output: result.stdout};
...
export function lsRemote(
	remote: string,
	ref?: string,
): {sha: string; ref: string} | undefined {
