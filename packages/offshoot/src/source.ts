...
export type Provider = "github" | "gitlab" | "bitbucket" | "sourcehut" | "local";
...
const HOSTED: Record<string, {prefix: string; url: (repo: string) => string}> = {
	...
	bitbucket: {prefix: "bitbucket", url: (r) => `https://bitbucket.org/${r}.git`},
...
export function parseSource(input: string): ParsedSource {
	...
	const looksLocal =
		provider === "local" || text.startsWith(".") || text.startsWith("/") || text.startsWith("~");

	if (looksLocal) {
		const path = resolve(text.startsWith("~") ? text.replace("~", process.env.HOME ?? "~") : text);
		return {
			id: `file:${path}`,
			provider: "local",
			repo: path,
...
export async function resolveRef(source: ParsedSource, ref?: string): Promise<ResolvedRef> {
	const wanted = ref ?? source.ref;

	if (wanted && FULL_SHA.test(wanted)) return {sha: wanted.toLowerCase()};

	if (source.provider === "local") {
		const rev = gitTry(["rev-parse", `${wanted ?? "HEAD"}^{commit}`], source.repo);
		if (rev.status !== 0) {
			throw new Error(`Cannot resolve "${wanted ?? "HEAD"}" in local template ${source.repo}.`);
		}
...
async function resolveViaGithubApi(source: ParsedSource, ref: string): Promise<string | undefined> {
	if (source.provider !== "github") return undefined;
	try {
		const headers: Record<string, string> = {Accept: "application/vnd.github+json"};
		const token = process.env.GITHUB_TOKEN ?? process.env.GH_TOKEN;
		if (token) headers.Authorization = `Bearer ${token}`;
		const res = await fetch(`https://api.github.com/repos/${source.repo}/commits/${ref}`, {headers});
...
export async function defaultTrack(source: ParsedSource): Promise<string | undefined> {
	if (source.provider === "local") {
...
export async function downloadTemplate(source: ParsedSource, sha: string): Promise<string> {
	const dir = mkdtempSync(join(tmpdir(), "offshoot-fetch-"));

	if (source.provider === "local") {
		if (!existsSync(join(source.repo, ".git")) && !existsSync(join(source.repo, "HEAD"))) {
	...
	await giget(`${source.gigetSource}#${sha}`, {dir, force: true, forceClean: true, silent: true});
	return dir;
}
