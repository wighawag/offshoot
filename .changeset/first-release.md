...
- Three composable transform strategies: `rename` (the default, token replacement across every case variant, in contents *and* paths), `patterns` (context-anchored pairs), and `template` (opt-in Eta placeholders, scoped to a glob).
...
- A round-trip uniqueness gate runs on every scaffold *and* every update, refusing a name that is not safely replaceable without `--force`.
